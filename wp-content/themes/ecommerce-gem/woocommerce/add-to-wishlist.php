...
<div class="yith-wcwl-add-to-wishlist add-to-wishlist-<?php
...
echo $product_id ?>">
	...
	<?php else: ?>
		<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_notice' => 'true', 'add_to_wishlist' => $product_id ), get_permalink( wc_get_page_id( 'myaccount' ) ) ) )?>" rel="nofollow" class="<?php echo
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		str_replace( 'add_to_wishlist', '', $link_classes ) ?>" >
			<?php
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo $icon ?>
			<?php
