...
if ( ! class_exists( 'eCommerce_Gem_About' ) ) {
	...
	class eCommerce_Gem_About {
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' .
			/* translators: 1: Theme name, 2: Theme page link */
			sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'ecommerce-gem' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' .
			/* translators: Theme name */
			sprintf( esc_html__( 'Get started with %s', 'ecommerce-gem' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $this->tabs ) ) {
					...
					wp_unslash( $_GET['tab'] ) : 'getting_started';
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '&tab=' .
						// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						$tab_key . '" class="nav-tab ' . ( $active_tab === $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'ecommerce-gem-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '2.1.7' );
				wp_enqueue_script( 'ecommerce-gem-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '2.1.7' );
				$js_vars = array(
				...
				if ( ! empty( $getting_started ) ) {
					...
					foreach ( $getting_started as $getting_started_item ) {
						...
						if ( ! empty( $getting_started_item['button_link'] ) && ! empty( $getting_started_item['button_label'] ) ) {
							...
							echo '<a target="' .
							// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							$button_new_tab . '" href="' . esc_url( $getting_started_item['button_link'] ) . '"class="' . esc_attr( $button_class ) . '">' . esc_html( $getting_started_item['button_label'] ) . '</a>';
							echo '</p>';
						}
				...
				if ( ! empty( $support_steps ) ) {
					...
					foreach ( $support_steps as $support_step ) {
						...
						if ( ! empty( $support_step['button_link'] ) && ! empty( $support_step['button_label'] ) ) {
							...
							echo '<a target="' .
							// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							$button_new_tab . '" href="' . esc_url( $support_step['button_link'] ) . '" class="' . esc_attr( $button_class ) . '">' . esc_html( $support_step['button_label'] ) . '</a>';
							echo '</p>';
						}
		...
		public function dismiss_recommended_action_callback() {
			...
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			esc_attr( wp_unslash( $_GET['todo'] ) ) : '';
			$action_id = ( isset( $_GET['id'] ) ) ?
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			esc_attr( wp_unslash( $_GET['id'] ) ) : '';
			$wpnonce   = ( isset( $_GET['_wpnonce'] ) ) ?
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			esc_attr( wp_unslash( $_GET['_wpnonce'] ) ) : '';
